...
class NetTest(BitcoinTestFramework):
    ...
    def test_addpeeraddress(self):
        ...
        assert_equal(node.addpeeraddress(address="1.2.3.4", port=8555), {"success": True})
        addrs = node.getnodeaddresses(count=0)
        assert_equal(len(addrs), 1)
        assert_equal(addrs[0]["address"], "1.2.3.4")
        assert_equal(addrs[0]["port"], 8555)

        self.log.debug("Test that adding the same address again when already present fails")
        assert_equal(node.addpeeraddress(address="1.2.3.4", port=8555), {"success": False})
